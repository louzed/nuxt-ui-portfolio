<template>
  <header class="sticky top-0 z-40 h-16 bg-default border-muted border-b shadow-none">
    <UContainer class="hidden lg:grid grid-cols-[1fr_auto_1fr] h-full items-center !px-8">
      ...
      <div class="flex items-center justify-center w-10 mx-auto">
    ...
    <UContainer class="flex lg:hidden items-center justify-between h-full !px-4">
      ...
      <div class="flex items-center gap-2">
        <AppColorMode />
        <UButton icon="i-heroicons-bars-3-20-solid" aria-label="Menü öffnen" variant="ghost" color="gray" size="xl"
          @click="isMobileMenuOpen = true" />
      </div>
    </UContainer>

    <UDrawer v-model:open="isMobileMenuOpen">
      <template #header>
...
// Shared state for active section highlighting
const activeSectionId = useState<string>('activeSection');

// State for controlling the mobile menu drawer
const isMobileMenuOpen = ref(false);

// Import AppSettings component if it's local and not globally registered
